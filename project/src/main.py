...
* Functions: sendTodatabase(),drawPred(),postprocess(),intermediate_detections(),process()

* Global Variables: PEOPLE_DETECTED, COUNTER, TIME_LIMIT
...
def sendTodatabase(PEOPLE_DETECTED):
    print("sendTodatabase() called")
    data  = waypoint_navigationvehicle.location.global_frame
    longitude = data.split(',')[1].split('=')[1]
    latitude = data.split(',')[0].split('=')[1]
    spreadsheet.sendData(PEOPLE_DETECTED,longitude,latitude)
...
def process(args):
    ...
    while stream.isOpened():
        ...
        result = [i for i in objects_list if i.startswith('person_')]
        #print("NO of persons",result)
        global PEOPLE_DETECTED
        TEMP_PEOPLE_DETECTED = len(result)
        if TEMP_PEOPLE_DETECTED > PEOPLE_DETECTED:
            PEOPLE_DETECTED = TEMP_PEOPLE_DETECTED
        ...
        diffrence_time = LATER_TIME - NOW_TIME
        print("diffrence_time : ",diffrence_time)
        # Exit if ESC pressed    
        if k == 27 : break
        if diffrence_time > TIME_LIMIT :
            print("Breaking from main")
            sendTodatabase(PEOPLE_DETECTED)
            break
